# modified by Yichao, to design for short DNA sequences
...
SEQUENCE_LENGTH = 196_608 # not used
BIN_SIZE = 128 # not used
TARGET_LENGTH = 6
...
class Enformer(snt.Module):
  ...
  def __init__(self,
    ...
    self.liyc_init = snt.initializers.VarianceScaling(scale=1,mode="fan_avg",distribution="uniform")
    # pylint: disable=g-complex-comprehension,g-long-lambda,cell-var-from-loop
    heads_channels = {'yeast': 1}
    dropout_rate = 0.4 # all the input sequence seems to be important, because it is only 110bp
    ...
    def conv_block(filters, width=1, w_init=self.liyc_init, name='conv_block', **kwargs):
    ...
    stem = Sequential(lambda: [
        snt.Conv1D(channels//4, 5), # typical core motif is only 3 or 4
        Residual(conv_block(channels//4, 1, name='pointwise_conv_block')),
        pooling_module("max2", pool_size=2),
    ], name='stem')

    # filter_list = exponential_linspace_int(start=channels // 2, end=channels,
                                           # num=6, divisible_by=128)
    filter_list = exponential_linspace_int(start=channels // 4, end=channels,
                                           num=4, divisible_by=32)
    conv_tower = Sequential(lambda: [
        Sequential(lambda: [
            conv_block(num_filters, 4), # changed from 5 to 2, need to capture small differences 
    ...
    final_pointwise = Sequential(lambda: [
        conv_block(channels , 1),
        pooling_module("max", pool_size=2),
        snt.Dropout(dropout_rate/8 ),
        gelu], name='final_pointwise')
        # tf.nn.relu], name='final_pointwise')
    ...
    with tf.name_scope('heads'):
      self._heads = {
          head: Sequential(
              lambda: [snt.Flatten(),snt.Linear(1,name="output"),tf.nn.softplus], # similar to dense-dense last part of a CNN
              # lambda: [snt.Flatten(),snt.Dropout(dropout_rate),snt.Linear(100,name="dense1",w_init=snt.initializers.RandomNormal(stddev=0.01)),tf.nn.relu,snt.Linear(1,name="output"),tf.nn.softplus], # similar to dense-dense last part of a CNN
              name=f'head_{head}')
...
class Sequential(snt.Module):
  ...
  def __call__(self, inputs: tf.Tensor, is_training: bool, **kwargs):
    outputs = inputs
    # print ("Sequential",self.name,"Input",inputs.shape)
    ...
    # print ("Sequential",self.name,"Output",outputs.shape)
    return outputs
...
def pooling_module(kind, pool_size):
  ...
  if kind == 'attention':
    ...
    # return tf.keras.layers.MaxPool1D(pool_size=pool_size, padding='same', strides=1) # try not to drop information
    return tf.keras.layers.MaxPool1D(pool_size=pool_size, padding='same') # try not to drop information
  elif kind == 'max2':
    return tf.keras.layers.MaxPool1D(pool_size=pool_size, padding='same', strides=1) # try not to drop information
  else:
